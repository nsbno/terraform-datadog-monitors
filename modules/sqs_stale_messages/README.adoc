= SQS Stale Messages Monitoring
:!toc-title:
:!toc-placement:
:toc:

Monitor for stale messages in a SQS queue based on the approximate age of the oldest message.
...
module "datadog_sqs_stale_messages" {
  source = "github.com/nsbno/terraform-datadog-monitors//modules/sqs_stale_messages?ref=x.y.z"
...
module "datadog_sqs_stale_messages" {
  ...
  threshold         = 600  # Critical at 600 seconds (10 minutes)
  warning_threshold = 300  # Warning at 300 seconds (5 minutes)
  period            = "5m"
