...
resource "datadog_monitor" "sqs_stale_messages" {
  ...
  query   = "avg(last_${var.period}):avg:aws.sqs.approximate_age_of_oldest_message{queuename:${lower(var.queue_name)}} by {queuename,region} > ${var.threshold}"
  message = var.workflow_to_attach != null ? var.workflow_to_attach : <<-EOT
  @slack-${var.slack_channel_to_notify}

  {{#is_alert}}
  The approximate age of the oldest message in ${var.queue_name} is outside of the typical range.
  In the last ${var.period} the average age was {{value}} seconds.
  {{/is_alert}}
  EOT
