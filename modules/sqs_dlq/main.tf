...
resource "datadog_monitor" "messages_in_dql" {
  ...
  query   = "sum(last_${var.period}):sum:aws.sqs.approximate_number_of_messages_visible{queuename:${lower(var.queue_name)}} >= ${var.threshold}"
  ...
  The last ${var.period} there have been approximately {{value}} messages visible.
  {{/is_alert}}

  {{#is_recovery}}
  No messages have been visible in the last ${var.period}.
  {{/is_recovery}}
  EOT
