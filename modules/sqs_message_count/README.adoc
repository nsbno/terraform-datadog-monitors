...
Monitor for high number of visible messages in a SQS queue based on the approximate number of messages visible.
...
module "datadog_sqs_message_count" {
  source = "github.com/nsbno/terraform-datadog-monitors//modules/sqs_message_count?ref=x.y.z"
...
module "datadog_sqs_message_count" {
  ...
  threshold         = 100  # Critical at 100 messages
  warning_threshold = 50   # Warning at 50 messages
  period            = "5m"
