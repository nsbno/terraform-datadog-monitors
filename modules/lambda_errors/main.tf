...
resource "datadog_monitor" "lambda_errors" {
  name = "${local.display_name}: ${var.function_name}: Lambda Errors"
  type = "query alert"
  ...
  query   = "sum(last_${var.period}):sum:aws.lambda.errors{functionname:${lower(var.function_name)}}.as_count() >= ${var.threshold}"
  message = var.workflow_to_attach != null ? var.workflow_to_attach : <<-EOT
  @slack-${var.slack_channel_to_notify}

  {{#is_alert}}
    ${local.display_name}: Lambda function ${var.function_name} has {{value}} errors in the last ${var.period}.
  {{/is_alert}}

  {{#is_recovery}}
    ${local.display_name}: Lambda function ${var.function_name} error rate has returned to normal.
  {{/is_recovery}}
  EOT
